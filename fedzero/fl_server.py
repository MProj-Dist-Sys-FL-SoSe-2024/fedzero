...
from fedzero.config import CRITICAL_LEARNING_OPTIMISATION, CRITICAL_LEARNING_THRESHOLD
...
class FedZeroServer(Server):
    ...
    def fit(self, num_rounds: int, timeout: Optional[float]) -> History:
        ...
        for current_round in range(1, num_rounds + 1):
            ...
            while True:
                ...
                res_fit = self.fit_round_ra(metrics=self._last_metrics, server_round=current_round, now=now,
                                            timeout=timeout)
    ...
    def fit_round_ra(self, metrics: dict[str, Any], server_round: int, now: datetime, timeout: Optional[float]) -> \
    Optional[
        Tuple[Optional[Parameters], Dict, FitResultsAndFailures, Dict[str, int], datetime]]:

        # if CRITICAL_LEARNING_OPTIMISATION is enabled and the local_weighted_train_loss_delta_ema is above
        # the CRITICAL_LEARNING_THRESHOLD, modify the selection to allow for more clients to participate (also not
        # green clients)
        # if (CRITICAL_LEARNING_OPTIMISATION
        #         and ((metrics.get("local_weighted_train_loss_delta_ema") is None)   # currently crashes because metrics is None and get() cannot be executed
        ...
        participation, round_duration = execute_round(self.power_domain_api, self.client_load_api, selection,
                                                      self.min_epochs, self.max_epochs)
        log(DEBUG,
